...
static ngx_int_t ngx_ssl_stapling_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, X509 *cert, ngx_str_t *file,
...
static ngx_int_t ngx_ssl_stapling_responder(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_ssl_stapling_t *staple,
                                            ngx_str_t *responder);
...
ngx_int_t ngx_ssl_stapling(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *file, ngx_str_t *responder, ngx_uint_t verify) {
    ...
    for (cert = SSL_CTX_get_ex_data(ssl->ctx, ngx_ssl_certificate_index); cert;
...
static ngx_int_t ngx_ssl_stapling_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl, X509 *cert, ngx_str_t *file,
    ...
    staple->cert = cert;
    staple->name = X509_get_ex_data(staple->cert, ngx_ssl_certificate_name_index);
...
static ngx_int_t ngx_ssl_stapling_file(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_ssl_stapling_t *staple, ngx_str_t *file) {
    ...
    if (response == NULL) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "d2i_OCSP_RESPONSE_bio(\"%s\") failed", file->data);
        BIO_free(bio);
        return NGX_ERROR;
    }
    ...
    if (len <= 0) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "i2d_OCSP_RESPONSE(\"%s\") failed", file->data);
        goto failed;
    }
...
static ngx_int_t ngx_ssl_stapling_issuer(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_ssl_stapling_t *staple) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ssl->log, 0, "SSL get issuer: %d extra certs", n);
    ...
    if (store_ctx == NULL) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "X509_STORE_CTX_new() failed");
        return NGX_ERROR;
    }

    if (X509_STORE_CTX_init(store_ctx, store, NULL, NULL) == 0) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "X509_STORE_CTX_init() failed");
    ...
    if (rc == -1) {
        ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "X509_STORE_CTX_get1_issuer() failed");
        X509_STORE_CTX_free(store_ctx);
        return NGX_ERROR;
    }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ssl->log, 0, "SSL get issuer: found %p in cert store", issuer);
...
ngx_int_t ngx_ssl_stapling_resolver(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_resolver_t *resolver,
                                    ngx_msec_t resolver_timeout) {
    X509 *cert;
    ngx_ssl_stapling_t *staple;
...
static void ngx_ssl_stapling_update(ngx_ssl_stapling_t *staple) {
    ...
    if (staple->host.len == 0 || staple->loading || staple->refresh >= ngx_time()) {
        return;
    }
...
static void ngx_ssl_stapling_ocsp_handler(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    if (ctx->status != V_OCSP_CERTSTATUS_GOOD) {
        ngx_log_error(NGX_LOG_ERR, ctx->log, 0, "certificate status \"%s\" in the OCSP response",
...
ngx_int_t ngx_ssl_ocsp(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *responder, ngx_uint_t depth,
                       ngx_shm_zone_t *shm_zone) {
    ngx_url_t u;
...
ngx_int_t ngx_ssl_ocsp_validate(ngx_connection_t *c) {
    ...
    if (ocsp->certs == NULL) {
        ...
        if (store == NULL) {
            ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "SSL_CTX_get_cert_store() failed");
            X509_free(cert);
            return NGX_ERROR;
        }
        ...
        store_ctx = X509_STORE_CTX_new();
        if (store_ctx == NULL) {
            ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "X509_STORE_CTX_new() failed");
            X509_free(cert);
            return NGX_ERROR;
        }
        ...
        if (X509_STORE_CTX_init(store_ctx, store, cert, chain) == 0) {
            ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "X509_STORE_CTX_init() failed");
            X509_STORE_CTX_free(store_ctx);
            X509_free(cert);
            return NGX_ERROR;
        }
        ...
        if (ocsp->certs == NULL) {
            ngx_ssl_error(NGX_LOG_ERR, c->log, 0, "X509_STORE_CTX_get1_chain() failed");
            X509_STORE_CTX_free(store_ctx);
            X509_free(cert);
            return NGX_ERROR;
        }
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl ocsp validate, certs:%d", sk_X509_num(ocsp->certs));
...
static void ngx_ssl_ocsp_validate_next(ngx_connection_t *c) {
    ...
    for (;;) {
        if (ocsp->ncert == n - 1 || (ocf->depth == 2 && ocsp->ncert == 1)) {
            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl ocsp validated, certs:%ui", ocsp->ncert);
            rc = NGX_OK;
            goto done;
        }

        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "ssl ocsp validate cert:%ui", ocsp->ncert);
        ...
        if (ctx->status != V_OCSP_CERTSTATUS_GOOD) {
            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cached status \"%s\"",
                           OCSP_cert_status_str(ctx->status));
...
static ngx_int_t ngx_ssl_ocsp_responder(ngx_connection_t *c, ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    if (aia == NULL) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "no OCSP responder URL in certificate");
        return NGX_ERROR;
    }
    ...
    if (ngx_parse_url(ctx->pool, &u) != NGX_OK) {
        if (u.err) {
            ngx_log_error(NGX_LOG_ERR, c->log, 0, "%s in OCSP responder \"%V\" in certificate", u.err, &u.url);
        }
    ...
    if (u.host.len == 0) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, "empty host in OCSP responder in certificate");
        return NGX_ERROR;
    }
...
static void ngx_ssl_ocsp_request(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp request");
    ...
    if (ctx->resolver) {
        ...
        if (resolve == NGX_NO_RESOLVER) {
            if (ctx->naddrs == 0) {
                ngx_log_error(NGX_LOG_ERR, ctx->log, 0, "no resolver defined to resolve %V", &ctx->host);
...
static void ngx_ssl_ocsp_resolve_handler(ngx_resolver_ctx_t *resolve) {
    ...
    if (resolve->state) {
        ngx_log_error(NGX_LOG_ERR, ctx->log, 0, "%V could not be resolved (%i: %s)", &resolve->name, resolve->state,
...
#if (NGX_DEBUG)
    {
        ...
        for (i = 0; i < resolve->naddrs; i++) {
            addr.len =
                ngx_sock_ntop(resolve->addrs[i].sockaddr, resolve->addrs[i].socklen, text, NGX_SOCKADDR_STRLEN, 0);

            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "name was resolved to %V", &addr);
        }
    }
#endif
...
static void ngx_ssl_ocsp_connect(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp connect %ui/%ui", ctx->naddr, ctx->naddrs);
    ...
    if (ctx->timeout) {
        ngx_event_add_timer(ctx->peer.connection->read, ctx->timeout);
        ngx_event_add_timer(ctx->peer.connection->write, ctx->timeout);
    }
...
static void ngx_ssl_ocsp_write_handler(ngx_event_t *wev) {
    ...
    n = ngx_io.send(c, ctx->request->pos, size);
    ...
    if (!wev->timer_set && ctx->timeout) {
        ngx_event_add_timer(wev, ctx->timeout);
    }
}
...
static void ngx_ssl_ocsp_read_handler(ngx_event_t *rev) {
    ...
    for (;;) {
        ...
        n = ngx_io.recv(c, ctx->response->last, size);
...
static ngx_int_t ngx_ssl_ocsp_create_request(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    if (ocsp == NULL) {
        ngx_ssl_error(NGX_LOG_CRIT, ctx->log, 0, "OCSP_REQUEST_new() failed");
        return NGX_ERROR;
    }
    ...
    if (OCSP_request_add0_id(ocsp, id) == NULL) {
        ngx_ssl_error(NGX_LOG_CRIT, ctx->log, 0, "OCSP_request_add0_id() failed");
        OCSP_CERTID_free(id);
        goto failed;
    }
    ...
    if (len <= 0) {
        ngx_ssl_error(NGX_LOG_CRIT, ctx->log, 0, "i2d_OCSP_REQUEST() failed");
        goto failed;
    }
    ...
    if (len <= 0) {
        ngx_ssl_error(NGX_LOG_EMERG, ctx->log, 0, "i2d_OCSP_REQUEST() failed");
        goto failed;
    }
    ...
    escape = ngx_escape_uri(NULL, base64.data, base64.len, NGX_ESCAPE_URI_COMPONENT);

    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp request length %z, escape %d", base64.len, (int)escape);
...
static ngx_int_t ngx_ssl_ocsp_process_status_line(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    if (rc == NGX_OK) {
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp status %ui \"%*s\"", ctx->code,
...
static ngx_int_t ngx_ssl_ocsp_parse_status_line(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp process status line");
    ...
    for (;;) {
        ...
        if (rc == NGX_OK) {
            ngx_log_debug4(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp header \"%*s: %*s\"",
                           ctx->header_name_end - ctx->header_name_start, ctx->header_name_start,
                           ctx->header_end - ctx->header_start, ctx->header_start);
            ...
            if (len == sizeof("Content-Type") - 1 &&
                ...
                if (len != sizeof("application/ocsp-response") - 1 ||
                    ngx_strncasecmp(ctx->header_start, (u_char *)"application/ocsp-response",
                                    sizeof("application/ocsp-response") - 1) != 0) {
...
static ngx_int_t ngx_ssl_ocsp_verify(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    if (n != OCSP_RESPONSE_STATUS_SUCCESSFUL) {
        ngx_log_error(NGX_LOG_ERR, ctx->log, 0, "OCSP response not successful (%d: %s)", n,
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp response, %s, %uz", OCSP_cert_status_str(ctx->status),
                   len);
...
ngx_int_t ngx_ssl_ocsp_cache_init(ngx_shm_zone_t *shm_zone, void *data) {
    ...
    ngx_rbtree_init(&cache->rbtree, &cache->sentinel, ngx_str_rbtree_insert_value);
    ...
    ngx_sprintf(shpool->log_ctx, " in OCSP cache \"%V\"%Z", &shm_zone->shm.name);
...
static ngx_int_t ngx_ssl_ocsp_cache_lookup(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    node = (ngx_ssl_ocsp_cache_node_t *)ngx_str_rbtree_lookup(&cache->rbtree, &ctx->key, hash);
    ...
    if (node) {
        if (node->valid > ngx_time()) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cache hit, %s",
                           OCSP_cert_status_str(ctx->status));
        ...
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cache expired");
...
static ngx_int_t ngx_ssl_ocsp_cache_store(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp cache store, valid:%T", valid - now);
    ...
    node = ngx_slab_calloc_locked(shpool, sizeof(ngx_ssl_ocsp_cache_node_t) + ctx->key.len);
    if (node == NULL) {
        if (!ngx_queue_empty(&cache->expire_queue)) {
            ...
            node = ngx_slab_alloc_locked(shpool, sizeof(ngx_ssl_ocsp_cache_node_t) + ctx->key.len);
        }

        if (node == NULL) {
            ngx_shmtx_unlock(&shpool->mutex);
            ngx_log_error(NGX_LOG_ALERT, ctx->log, 0, "could not allocate new entry%s", shpool->log_ctx);
            return NGX_ERROR;
        }
    }
...
static ngx_int_t ngx_ssl_ocsp_create_key(ngx_ssl_ocsp_ctx_t *ctx) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ctx->log, 0, "ssl ocsp key %xV", &ctx->key);
...
ngx_int_t ngx_ssl_ocsp(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *responder, ngx_uint_t depth,
                       ngx_shm_zone_t *shm_zone) {
    ngx_log_error(NGX_LOG_EMERG, ssl->log, 0, "\"ssl_ocsp\" is not supported on this platform");
