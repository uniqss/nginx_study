...
static ngx_command_t ngx_epoll_commands[] = {
    ...
    {ngx_string("epoll_events"), NGX_EVENT_CONF | NGX_CONF_TAKE1, ngx_conf_set_num_slot, 0,
     offsetof(ngx_epoll_conf_t, events), NULL},
...
static ngx_event_module_t ngx_epoll_module_ctx = {&epoll_name,
...
static void ngx_epoll_test_rdhup(ngx_cycle_t *cycle) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "testing the EPOLLRDHUP flag: %s",
...
static ngx_int_t ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags) {
    ...
    for (i = 0; i < events; i++) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "epoll: fd:%d ev:%04XD d:%p", c->fd, revents,
...
static void ngx_epoll_eventfd_handler(ngx_event_t *ev) {
    ...
    while (ready) {
        ...
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, ev->log, 0, "io_getevents: %d", events);

        if (events > 0) {
            ready -= events;

            for (i = 0; i < events; i++) {
                ngx_log_debug4(NGX_LOG_DEBUG_EVENT, ev->log, 0, "io_event: %XL %XL %L %L", event[i].data, event[i].obj,
