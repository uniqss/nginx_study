...
ngx_int_t ngx_enable_accept_events(ngx_cycle_t *cycle) {
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_event_actions.add(c->read, NGX_READ_EVENT, 0) == NGX_ERROR) {
            return NGX_ERROR;
        }
    }
...
static ngx_int_t ngx_disable_accept_events(ngx_cycle_t *cycle, ngx_uint_t all) {
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (ngx_event_actions.del(c->read, NGX_READ_EVENT, NGX_DISABLE_EVENT) == NGX_ERROR) {
            return NGX_ERROR;
        }
    }
...
static void ngx_reorder_accept_events(ngx_listening_t *ls) {
    ...
    if (c->requests++ % 16 != 0 && ngx_accept_disabled <= 0) {
        return;
    }
    ...
    if (ngx_event_actions.add(c->read, NGX_READ_EVENT, NGX_EXCLUSIVE_EVENT) == NGX_ERROR) {
        return;
    }
}
