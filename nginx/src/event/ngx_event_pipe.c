...
static ngx_int_t ngx_event_pipe_read_upstream(ngx_event_pipe_t *p) {
    ...
    if (p->aio) {
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe read upstream: aio");
        return NGX_AGAIN;
    }

    if (p->writing) {
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe read upstream: writing");
    ...
    for (;;) {
        ...
        if (p->preread_bufs) {
            ...
            if (p->upstream->read->available == 0 && p->upstream->read->pending_eof
#if (NGX_SSL)
                && !p->upstream->ssl
#endif
            ) {
                ...
                if (p->upstream->read->kq_errno) {
                    ...
                    ngx_log_error(NGX_LOG_ERR, p->log, p->upstream->read->kq_errno,
    ...
    for (cl = p->busy; cl; cl = cl->next) {
        ngx_log_debug8(NGX_LOG_DEBUG_EVENT, p->log, 0,
                       ...
                       "%p, pos %p, size: %z "
                       "file: %O, size: %O",
                       (cl->buf->shadow ? 1 : 0), cl->buf->temporary, cl->buf->in_file, cl->buf->start, cl->buf->pos,
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe length: %O", p->length);
    ...
    for (;;) {
        ...
        for (;;) {
            if (p->out) {
                ...
                ngx_log_debug3(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe write buf ls:%d %p %z", cl->buf->last_shadow,
...
static ngx_int_t ngx_event_pipe_write_chain_to_temp_file(ngx_event_pipe_t *p) {
    ...
    if (!p->cacheable) {
        ...
        do {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_EVENT, p->log, 0, "pipe buf ls:%d %p, pos %p, size: %z", cl->buf->last_shadow,
                           cl->buf->start, cl->buf->pos, bsize);

            if (prev_last_shadow && ((size + bsize > p->temp_file_write_size) ||
...
ngx_int_t ngx_event_pipe_copy_input_filter(ngx_event_pipe_t *p, ngx_buf_t *buf) {
    ...
    if (p->upstream_done) {
        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, p->log, 0, "input data after close");
        return NGX_OK;
    }
