...
static void ngx_signal_worker_processes(ngx_cycle_t *cycle, int signo) {
    ...
    for (i = 0; i < ngx_last_process; i++) {
        ngx_log_debug7(NGX_LOG_DEBUG_EVENT, cycle->log, 0, "child: %i %P e:%d t:%d d:%d r:%d j:%d", i,
                       ngx_processes[i].pid, ngx_processes[i].exiting, ngx_processes[i].exited,
                       ngx_processes[i].detached, ngx_processes[i].respawn, ngx_processes[i].just_spawn);
        ...
        if (ngx_processes[i].exited) {
            if (!ngx_processes[i].detached) {
                ...
                for (n = 0; n < ngx_last_process; n++) {
                    ...
                    ngx_log_debug3(NGX_LOG_DEBUG_CORE, cycle->log, 0, "pass close channel s:%i pid:%P to:%P", ch.slot,
                                   ch.pid, ngx_processes[n].pid);
            ...
            if (ngx_processes[i].respawn && !ngx_processes[i].exiting && !ngx_terminate && !ngx_quit) {
                if (ngx_spawn_process(cycle, ngx_processes[i].proc, ngx_processes[i].data, ngx_processes[i].name, i) ==
                    NGX_INVALID_PID) {
...
static void ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker) {
    ...
    if (ccf->rlimit_nofile != NGX_CONF_UNSET) {
        ...
        if (setrlimit(RLIMIT_NOFILE, &rlmt) == -1) {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, "setrlimit(RLIMIT_NOFILE, %i) failed",
                          ccf->rlimit_nofile);
        }
    }
    ...
    if (ngx_exiting) {
        ...
        for (i = 0; i < cycle->connection_n; i++) {
            if (c[i].fd != -1 && c[i].read && !c[i].read->accept && !c[i].read->channel && !c[i].read->resolver) {
                ngx_log_error(NGX_LOG_ALERT, cycle->log, 0, "*%uA open socket #%d left in connection %ui", c[i].number,
                              c[i].fd, i);
                ngx_debug_quit = 1;
            }
        }
...
static void ngx_channel_handler(ngx_event_t *ev) {
    ...
    for (;;) {
        ...
        if (ngx_event_flags & NGX_USE_EVENTPORT_EVENT) {
            if (ngx_event_actions.add(ev, NGX_READ_EVENT, 0) == NGX_ERROR) {
                return;
            }
        }
        ...
        switch (ch.command) {
            ...
            case NGX_CMD_CLOSE_CHANNEL:
                ...
                ngx_log_debug4(NGX_LOG_DEBUG_CORE, ev->log, 0, "close channel s:%i pid:%P our:%P fd:%d", ch.slot,
                               ch.pid, ngx_processes[ch.slot].pid, ngx_processes[ch.slot].channel[0]);
