...
ssize_t ngx_unix_recv(ngx_connection_t *c, u_char *buf, size_t size) {
    ...
    if (ngx_event_flags & NGX_USE_KQUEUE_EVENT) {
        ngx_log_debug3(NGX_LOG_DEBUG_EVENT, c->log, 0, "recv: eof:%d, avail:%d, err:%d", rev->pending_eof,
        ...
        if (rev->available == 0) {
            if (rev->pending_eof) {
                ...
                if (rev->kq_errno) {
                    ...
                    return ngx_connection_error(c, rev->kq_errno, "kevent() reported about an closed connection");
                }
    ...
    do {
        ...
        if (n > 0) {
            ...
            if (rev->available >= 0) {
                ...
                ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "recv: avail:%d", rev->available);
