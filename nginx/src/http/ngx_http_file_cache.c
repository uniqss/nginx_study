...
ngx_str_t ngx_http_cache_status[] = {ngx_string("MISS"),  ngx_string("BYPASS"),   ngx_string("EXPIRED"),
                                     ngx_string("STALE"), ngx_string("UPDATING"), ngx_string("REVALIDATED"),
                                     ngx_string("HIT")};
    ...
    if (ocache) {
        ...
        for (n = 0; n < NGX_MAX_PATH_LEVEL; n++) {
            if (cache->path->level[n] != ocache->path->level[n]) {
                ngx_log_error(NGX_LOG_EMERG, shm_zone->shm.log, 0, "cache \"%V\" had previously different levels",
                              &shm_zone->shm.name);
                return NGX_ERROR;
            }
        }
...
ngx_int_t ngx_http_file_cache_open(ngx_http_request_t *r) {
    ...
    if (ngx_open_cached_file(clcf->open_file_cache, &c->file.name, &of, r->pool) != NGX_OK) {
        switch (of.err) {
            ...
            default:
                ngx_log_error(NGX_LOG_CRIT, r->connection->log, of.err, ngx_open_file_n " \"%s\" failed",
                              c->file.name.data);
                return NGX_ERROR;
        }
    }
...
static ngx_int_t ngx_http_file_cache_lock(ngx_http_request_t *r, ngx_http_cache_t *c) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http file cache lock u:%d wt:%M", c->updating,
                   c->wait_time);
    ...
    ngx_event_add_timer(&c->wait_event, (timer > 500) ? 500 : timer);
...
static ngx_int_t ngx_http_file_cache_read(ngx_http_request_t *r, ngx_http_cache_t *c) {
    ...
    for (i = 0; i < c->keys.nelts; i++) {
        if (ngx_memcmp(p, key[i].data, key[i].len) != 0) {
            ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, "cache file \"%s\" has md5 collision",
                          c->file.name.data);
            return NGX_DECLINED;
        }
    ...
    if (h->vary_len > NGX_HTTP_CACHE_VARY_LEN) {
        ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, "cache file \"%s\" has incorrect vary length",
                      c->file.name.data);
        return NGX_DECLINED;
    }
    ...
    if (c->valid_sec < now) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http file cache expired: %i %T %T", rc, c->valid_sec,
                       now);
...
static void ngx_http_cache_aio_event_handler(ngx_event_t *ev) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "http file cache aio: \"%V?%V\"", &r->uri, &r->args);
...
static void ngx_http_file_cache_rbtree_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node,
    ...
    for (;;) {
        if (node->key < temp->key) {
            ...
            p = (ngx_memcmp(cn->key, cnt->key, NGX_HTTP_CACHE_KEY_LEN - sizeof(ngx_rbtree_key_t)) < 0) ? &temp->left
                                                                                                       : &temp->right;
        }
...
static ngx_int_t ngx_http_file_cache_reopen(ngx_http_request_t *r, ngx_http_cache_t *c) {
    ...
    if (c->secondary) {
        ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, "cache file \"%s\" has incorrect vary hash",
                      c->file.name.data);
        return NGX_DECLINED;
    }
...
void ngx_http_file_cache_update(ngx_http_request_t *r, ngx_temp_file_t *tf) {
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http file cache rename: \"%s\" to \"%s\"",
    ...
    if (rc == NGX_OK) {
        if (ngx_fd_info(tf->file.fd, &fi) == NGX_FILE_ERROR) {
            ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno, ngx_fd_info_n " \"%s\" failed",
                          tf->file.name.data);
...
void ngx_http_file_cache_update_header(ngx_http_request_t *r) {
    ...
    if (file.fd == NGX_INVALID_FILE) {
        ...
        if (err == NGX_ENOENT) {
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http file cache \"%s\" not found",
                           file.name.data);
            return;
        }
    ...
    if ((size_t)n != sizeof(ngx_http_file_cache_header_t)) {
        ngx_log_error(NGX_LOG_CRIT, r->connection->log, 0, ngx_read_file_n " read only %z of %z from \"%s\"", n,
                      sizeof(ngx_http_file_cache_header_t), file.name.data);
        goto done;
    }
...
void ngx_http_file_cache_free(ngx_http_cache_t *c, ngx_temp_file_t *tf) {
    ...
    if (c->temp_file) {
        if (tf && tf->file.fd != NGX_INVALID_FILE) {
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->file.log, 0, "http file cache incomplete: \"%s\"",
                           tf->file.name.data);

            if (ngx_delete_file(tf->file.name.data) == NGX_FILE_ERROR) {
                ngx_log_error(NGX_LOG_CRIT, c->file.log, ngx_errno, ngx_delete_file_n " \"%s\" failed",
                              tf->file.name.data);
            }
        }
    }
...
static time_t ngx_http_file_cache_forced_expire(ngx_http_file_cache_t *cache) {
    ...
    for (;;) {
        ...
        ngx_log_debug6(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0,
                       "http file cache forced expire: #%d %d %02xd%02xd%02xd%02xd", fcn->count, fcn->exists,
...
static time_t ngx_http_file_cache_expire(ngx_http_file_cache_t *cache) {
    ...
    for (;;) {
        ...
        ngx_log_debug6(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "http file cache expire: #%d %d %02xd%02xd%02xd%02xd",
...
static ngx_msec_t ngx_http_file_cache_manager(void *data) {
    ...
    for (;;) {
        ...
        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "http file cache size: %O c:%ui w:%i", size, count,
                       (ngx_int_t)watermark);
        ...
        if (size < cache->max_size && count < watermark) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "http file cache free: %O", free);
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, ngx_cycle->log, 0, "http file cache manager: %ui e:%M n:%M", cache->files,
                   elapsed, next);
...
char *ngx_http_file_cache_set_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    for (i = 2; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "min_free=", 9) == 0) {
            ...
            if (min_free < 0) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid min_free value \"%V\"", &value[i]);
                return NGX_CONF_ERROR;
            }
