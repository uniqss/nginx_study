...
static ngx_int_t ngx_http_ssi_evaluate_string(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t *text,
                                              ngx_uint_t flags);
...
static ngx_command_t ngx_http_ssi_filter_commands[] = {
    ...
    {ngx_string("ssi_ignore_recycled_buffers"),
     ...
     NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, ignore_recycled_buffers), NULL},
    ...
    {ngx_string("ssi_types"), NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE,
     ngx_http_types_slot, NGX_HTTP_LOC_CONF_OFFSET, offsetof(ngx_http_ssi_loc_conf_t, types_keys),
...
ngx_module_t ngx_http_ssi_filter_module = {NGX_MODULE_V1,
...
static ngx_http_ssi_param_t ngx_http_ssi_include_params[] = {
    {ngx_string("virtual"), NGX_HTTP_SSI_INCLUDE_VIRTUAL, 0, 0}, {ngx_string("file"), NGX_HTTP_SSI_INCLUDE_FILE, 0, 0},
    {ngx_string("wait"), NGX_HTTP_SSI_INCLUDE_WAIT, 0, 0},       {ngx_string("set"), NGX_HTTP_SSI_INCLUDE_SET, 0, 0},
    {ngx_string("stub"), NGX_HTTP_SSI_INCLUDE_STUB, 0, 0},       {ngx_null_string, 0, 0, 0}};
...
static ngx_int_t ngx_http_ssi_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    if (ctx == NULL || (ctx->shared && r == r->main) ||
        (in == NULL && ctx->buf == NULL && ctx->in == NULL && ctx->busy == NULL)) {
    ...
    if (ctx->wait) {
        if (r != r->connection->data) {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" non-active",
        ...
        if (ctx->wait->done) {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\" done",
                           &ctx->wait->uri, &ctx->wait->args);

            ctx->wait = NULL;

        } else {
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http ssi filter wait \"%V?%V\"", &ctx->wait->uri,
                           &ctx->wait->args);
    ...
    while (ctx->in || ctx->buf) {
        ...
        while (ctx->pos < ctx->buf->last) {
            ...
            ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "parse: %i, looked: %uz %p-%p", rc, ctx->looked,
            ...
            if (rc == NGX_OK) {
                ...
                if (cmd->conditional && (ctx->conditional == 0 || ctx->conditional > cmd->conditional)) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "invalid context of SSI command: \"%V\"",
                                  &ctx->command);
                    goto ssi_error;
                }

                if (ctx->params.nelts > NGX_HTTP_SSI_MAX_PARAMS) {
                    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "too many SSI command parameters: \"%V\"",
                                  &ctx->command);
                    goto ssi_error;
                }
                ...
                for (i = 0; i < ctx->params.nelts; i++) {
                    for (prm = cmd->params; prm->name.len; prm++) {
                        if (param[i].key.len != prm->name.len ||
                            ngx_strncmp(param[i].key.data, prm->name.data, prm->name.len) != 0) {
                            continue;
                        }
        ...
        switch (state) {
                ...
                switch (ch) {
                    ...
                    default:
                        if (ctx->command.len == NGX_HTTP_SSI_COMMAND_LEN) {
                            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "the \"%V%c...\" SSI command is too long",
                                          &ctx->command, ch);
            ...
            case ssi_comment_end0_state:
                switch (ch) {
                    ...
                    default:
                        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                                      "unexpected \"%c\" symbol in \"%V\" SSI command", ch, &ctx->command);
                        state = ssi_error_state;
                        break;
                }
...
static ngx_int_t ngx_http_ssi_regex_match(ngx_http_request_t *r, ngx_str_t *pattern, ngx_str_t *str) {
    ...
    if (rc < NGX_REGEX_NO_MATCHED) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, ngx_regex_exec_n " failed: %d on \"%V\" using \"%V\"", rc,
                      str, pattern);
        return NGX_HTTP_SSI_ERROR;
    }
    ...
    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "the using of the regex \"%V\" in SSI requires PCRE library",
                  pattern);
    return NGX_HTTP_SSI_ERROR;
...
static ngx_int_t ngx_http_ssi_include(ngx_http_request_t *r, ngx_http_ssi_ctx_t *ctx, ngx_str_t **params) {
    ...
    if (uri && file) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "inclusion may be either virtual=\"%V\" or file=\"%V\"", uri,
                      file);
        return NGX_HTTP_SSI_ERROR;
    }
