...
static ngx_int_t ngx_http_variable_headers_internal(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data,
                                                    u_char sep);
...
static ngx_int_t ngx_http_variable_proxy_protocol_port(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                       uintptr_t data);
static ngx_int_t ngx_http_variable_proxy_protocol_tlv(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                      uintptr_t data);
...
static ngx_int_t ngx_http_variable_sent_transfer_encoding(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                          uintptr_t data);
...
static ngx_int_t ngx_http_variable_connection_requests(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                       uintptr_t data);
static ngx_int_t ngx_http_variable_connection_time(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data);
...
static ngx_http_variable_t ngx_http_core_variables[] = {
    ...
    {ngx_string("http_user_agent"), NULL, ngx_http_variable_header, offsetof(ngx_http_request_t, headers_in.user_agent),
     0, 0},

    {ngx_string("http_referer"), NULL, ngx_http_variable_header, offsetof(ngx_http_request_t, headers_in.referer), 0,
     0},
    ...
    {ngx_string("http_cookie"), NULL, ngx_http_variable_cookies, offsetof(ngx_http_request_t, headers_in.cookie), 0, 0},

    {ngx_string("content_length"), NULL, ngx_http_variable_content_length, 0, 0, 0},

    {ngx_string("content_type"), NULL, ngx_http_variable_header, offsetof(ngx_http_request_t, headers_in.content_type),
     0, 0},
    ...
    {ngx_string("proxy_protocol_addr"), NULL, ngx_http_variable_proxy_protocol_addr,
     offsetof(ngx_proxy_protocol_t, src_addr), 0, 0},

    {ngx_string("proxy_protocol_port"), NULL, ngx_http_variable_proxy_protocol_port,
     offsetof(ngx_proxy_protocol_t, src_port), 0, 0},

    {ngx_string("proxy_protocol_server_addr"), NULL, ngx_http_variable_proxy_protocol_addr,
     offsetof(ngx_proxy_protocol_t, dst_addr), 0, 0},

    {ngx_string("proxy_protocol_server_port"), NULL, ngx_http_variable_proxy_protocol_port,
     offsetof(ngx_proxy_protocol_t, dst_port), 0, 0},

    {ngx_string("proxy_protocol_tlv_"), NULL, ngx_http_variable_proxy_protocol_tlv, 0, NGX_HTTP_VAR_PREFIX, 0},
    ...
    {ngx_string("uri"), NULL, ngx_http_variable_request, offsetof(ngx_http_request_t, uri), NGX_HTTP_VAR_NOCACHEABLE,
     0},

    {ngx_string("document_uri"), NULL, ngx_http_variable_request, offsetof(ngx_http_request_t, uri),
    ...
    {ngx_string("query_string"), NULL, ngx_http_variable_request, offsetof(ngx_http_request_t, args),
     NGX_HTTP_VAR_NOCACHEABLE, 0},
    ...
    {ngx_string("sent_http_link"), NULL, ngx_http_variable_header, offsetof(ngx_http_request_t, headers_out.link), 0,
     0},
    ...
    {ngx_string("connection_time"), NULL, ngx_http_variable_connection_time, 0, NGX_HTTP_VAR_NOCACHEABLE, 0},
...
static ngx_int_t ngx_http_variable_unknown_header_in(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                     uintptr_t data) {
    return ngx_http_variable_unknown_header(r, v, (ngx_str_t *)data, &r->headers_in.headers.part, sizeof("http_") - 1);
}


static ngx_int_t ngx_http_variable_unknown_header_out(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                      uintptr_t data) {
    return ngx_http_variable_unknown_header(r, v, (ngx_str_t *)data, &r->headers_out.headers.part,
                                            sizeof("sent_http_") - 1);
}
...
static ngx_int_t ngx_http_variable_unknown_trailer_out(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                       uintptr_t data) {
    return ngx_http_variable_unknown_header(r, v, (ngx_str_t *)data, &r->headers_out.trailers.part,
                                            sizeof("sent_trailer_") - 1);
}


ngx_int_t ngx_http_variable_unknown_header(ngx_http_request_t *r, ngx_http_variable_value_t *v, ngx_str_t *var,
...
static ngx_int_t ngx_http_variable_cookie(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    if (ngx_http_parse_multi_header_lines(r, r->headers_in.cookie, &s, &cookie) == NULL) {
        v->not_found = 1;
        return NGX_OK;
    }
...
static ngx_int_t ngx_http_variable_realpath_root(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    if (real == NULL) {
        ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno, ngx_realpath_n " \"%s\" failed", path.data);
        return NGX_ERROR;
    }
...
static ngx_int_t ngx_http_variable_sent_location(ngx_http_request_t *r, ngx_http_variable_value_t *v, uintptr_t data) {
    ...
    return ngx_http_variable_unknown_header(r, v, &name, &r->headers_out.headers.part, sizeof("sent_http_") - 1);
}
...
static ngx_int_t ngx_http_variable_connection_time(ngx_http_request_t *r, ngx_http_variable_value_t *v,
                                                   uintptr_t data) {
    u_char *p;
    ngx_msec_int_t ms;
...
ngx_int_t ngx_http_regex_exec(ngx_http_request_t *r, ngx_http_regex_t *re, ngx_str_t *s) {
    ...
    if (rc < 0) {
        ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, ngx_regex_exec_n " failed: %i on \"%V\" using \"%V\"", rc,
                      s, &re->name);
        return NGX_ERROR;
    }
    ...
    for (i = 0; i < re->nvariables; i++) {
...
#if (NGX_DEBUG)
        {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http regex set $%V to \"%v\"", &v[index].name,
                           vv);
        }
#endif
    }
