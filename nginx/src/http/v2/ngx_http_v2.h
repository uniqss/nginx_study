...
#define NGX_HTTP_V2_MAX_FIELD (127 + (1 << (NGX_HTTP_V2_INT_OCTETS - 1) * 7) - 1)
...
#define ngx_http_v2_write_uint32(p, s)                                                                              \
    ((p)[0] = (u_char)((s) >> 24), (p)[1] = (u_char)((s) >> 16), (p)[2] = (u_char)((s) >> 8), (p)[3] = (u_char)(s), \
     (p) + sizeof(uint32_t))
...
#define ngx_http_v2_write_name(dst, src, len, tmp) ngx_http_v2_string_encode(dst, src, len, tmp, 1)
#define ngx_http_v2_write_value(dst, src, len, tmp) ngx_http_v2_string_encode(dst, src, len, tmp, 0)
