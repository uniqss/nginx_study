...
ngx_int_t ngx_open_cached_file(ngx_open_file_cache_t *cache, ngx_str_t *name, ngx_open_file_info_t *of,
    ...
    if (file) {
        ...
        ngx_log_debug4(NGX_LOG_DEBUG_CORE, pool->log, 0, "retest open file: %s, fd:%d, c:%d, e:%d", file->name,
                       file->fd, file->count, file->err);
...
found:
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_CORE, pool->log, 0, "cached open file: %s, fd:%d, c:%d, e:%d, u:%d", file->name,
                   file->fd, file->count, file->err, file->uses);
...
static void ngx_open_file_add_event(ngx_open_file_cache_t *cache, ngx_cached_open_file_t *file,
    ...
    if (ngx_event_actions.add(file->event, NGX_VNODE_EVENT, NGX_ONESHOT_EVENT) != NGX_OK) {
...
static void ngx_close_cached_file(ngx_open_file_cache_t *cache, ngx_cached_open_file_t *file, ngx_uint_t min_uses,
                                  ngx_log_t *log) {
    ngx_log_debug5(NGX_LOG_DEBUG_CORE, log, 0, "close cached open file: %s, fd:%d, c:%d, u:%d, %d", file->name,
                   file->fd, file->count, file->uses, file->close);
...
static void ngx_open_file_del_event(ngx_cached_open_file_t *file) {
    ...
    (void)ngx_event_actions.del(file->event, NGX_VNODE_EVENT, file->count ? NGX_FLUSH_EVENT : NGX_CLOSE_EVENT);
