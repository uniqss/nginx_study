...
#if !(NGX_WIN32)
        {
            ...
            if (fi.st_uid != user) {
                if (chown((const char *)path[i]->name.data, user, -1) == -1) {
                    ngx_log_error(NGX_LOG_EMERG, cycle->log, ngx_errno, "chown(\"%s\", %d) failed", path[i]->name.data,
                                  user);
                    return NGX_ERROR;
                }
            }
...
ngx_int_t ngx_ext_rename_file(ngx_str_t *src, ngx_str_t *to, ngx_ext_rename_file_t *ext) {
    ...
    if (err == NGX_EXDEV) {
        ...
        if (ngx_copy_file(src->data, name, &cf) == NGX_OK) {
            ...
            ngx_log_error(NGX_LOG_CRIT, ext->log, ngx_errno, ngx_rename_file_n " \"%s\" to \"%s\" failed", name,
                          to->data);
